...
//       if (token) {
//         await storePushToken(token); // Pass the token to your backend to store it.
//       }
//     };
...
//       setNotification(notification); // Set the received notification
//     });
  
//     const responseListener = Notifications.addNotificationResponseReceivedListener(response => {
//       console.log(response); // Handle the notification response
//     });
...
//       shouldShowAlert: true,  // Show alert when notification arrives
//       shouldPlaySound: true,  // Enable sound
//       shouldSetBadge: true,   // Set app icon badge
//     }),
//   });
  
//   // Listener for receiving notifications when app is in foreground
//   useEffect(() => {
//     const notificationListener = Notifications.addNotificationReceivedListener(notification => {
...
//       console.log("Expo Push Token: ", token); // Confirm the token is generated
//     } else {
...
//   const storePushToken = async (username, token) => {
//     try {
//       const response = await axios.post('http://50.6.194.240:5000/api/store-push-token', {
//         username,  // Pass the username and token to the backend
//         pushToken: token,
//       });
...
//           await storePushToken(username, expoPushToken); 
//         } else {
...
//         // Navigate to PieChart screen first
//         navigation.navigate('PieChart', { username });

//         // After 3 seconds, navigate to ParentDashboard screen
//         setTimeout(() => {
...
//       }
//     } catch (error) {
//       console.error('Login error:', error.response?.data?.message || 'An unexpected error occurred. Please try again.');
...
//             >
//               <Ionicons name={showPassword ? 'eye-off' : 'eye'} size={24} color="lightgrey" />
//             </TouchableOpacity>
//           </View>
...
import React, { useState, useEffect, useContext } from 'react';
import { View, Image, TextInput, Button, Alert, StyleSheet, TouchableOpacity,Text,Modal } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { useNavigation } from '@react-navigation/native';
import axios from 'axios';
import { Ionicons } from '@expo/vector-icons'; 
import { UserContext } from './UserContext';
import * as Notifications from 'expo-notifications';
import * as Device from 'expo-device'; // Adjust the path if necessary
...
const ParentLogin = () => {
  ...
  useEffect(() => {
    const setupNotifications = async () => {
      const token = await registerForPushNotificationsAsync();
      if (token) {
        setExpoPushToken(token); // Save the token in state for later use
      }
    };
  ...
  const storePushToken = async (studentName, token) => {
    try {
      const response = await axios.post('http://50.6.194.240:5000/api/store-push-token', {
        studentName, 
        pushToken: token,
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Store the push token after successful login
          // Fetch the student name (already fetched earlier in useEffect)
      if (studentName && expoPushToken) {
        // Store the push token after successful login
        await storePushToken(studentName, expoPushToken);
      } else {
        console.warn('Student name or push token is missing. Cannot store token.');
      }
...
  };
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await axios.post('http://50.6.194.240:5000/api/resett-password', {
        username,
        newPassword,
      });
